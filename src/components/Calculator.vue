  ...
  <div class="p-3" style="max-width:400px; margin:50px auto;background: #234">
    ...
    <div class="row ">
      <div class="col-3 p-0" v-for="n  in calculatorElements" :key="n">
        <div class="lead text-white text-center m-1 py-3  bg-vue-dark rounded hover-class" :class="{'bg-vue-green':['C', '*', '/', '-','+',
...
export default {
  ...
  methods: {
    ...
    if(['*', '/', '-','+','%'].includes(n)){
        this.operatorValue = n;
        this.previousCalculatorValue = this.calculatorValue;
        this.calculatorValue = ''; 
    }
    if(n === '='){
      this.calculatorValue = eval(
        this.previousCalculatorValue + this.operatorValue + this.calculatorValue
      );
